...
export const TopNavbar: React.FC = () => {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full h-16 bg-white/70 dark:bg-slate-950/70 backdrop-blur-xl border-b border-slate-200/60 dark:border-slate-900/60 px-4 md:px-6 flex items-center justify-between transition-all">
        ...
        <div className="flex items-center gap-2 md:gap-4">
            ...
            {roleMenuOpen && (
              <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-850 p-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-wider text-slate-400 border-b border-slate-100 dark:border-slate-800/85">
                  Change Interactive Persona
                </div>
                {(['citizen', 'worker', 'admin', 'guest'] as UserRole[]).map((r) => (
                  <button
                    key={r}
                    onClick={() => {
          ...
          {user ? (
              ...
              {profileMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-850 p-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                  <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-850">
                    ...
                    <div className="text-[11px] text-slate-450 truncate">{user.email}</div>
                  </div>
                  <div className="py-1">
                    <a
                      href="/profile"
                      className="block px-3 py-2 rounded-xl text-xs font-medium text-slate-700 dark:text-slate-350 hover:bg-slate-100 dark:hover:bg-slate-800"
                    >
                      View Profile
                    </a>
                    <button
            ...
            <a
              href="/login"
              className="px-3.5 sm:px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-sm transition-colors"
            >
              Sign In
            </a>
          )}
