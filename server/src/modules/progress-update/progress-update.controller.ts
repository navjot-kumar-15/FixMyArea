...
import { ProgressUpdateResponseDto, PaginatedProgressUpdateResponseDto } from './dto/progress-update-response.dto';
...
export class ProgressUpdateController {
  ...
  @ApiResponse({ status: 201, type: ProgressUpdateResponseDto, description: 'The progress update has been successfully created.' })
  @ApiResponse({ status: 400, description: 'Invalid request data.' })
  async create(@Body() createProgressUpdateDto: CreateProgressUpdateDto) {
    try {
      const update = await this.progressUpdateService.create(createProgressUpdateDto);
      const mapped = ProgressUpdateMapper.toResponse(update);
      return CustomResponse.success(mapped, MESSAGES.PROGRESS_UPDATE.CREATED, 201);
    } catch (error) {
  ...
  @ApiResponse({ status: 200, type: PaginatedProgressUpdateResponseDto, description: 'All progress updates retrieved successfully.' })
  async findAll(@Query() filterProgressUpdateDto: FilterProgressUpdateDto) {
    try {
      const paginatedResult = await this.progressUpdateService.findAll(filterProgressUpdateDto);
      const mappedResult: PaginatedProgressUpdateResponseDto = {
        ...paginatedResult,
        data: ProgressUpdateMapper.toResponseList(paginatedResult.data),
      };
      return CustomResponse.success(mappedResult, MESSAGES.PROGRESS_UPDATE.FETCHED_ALL);
    } catch (error) {
      return CustomResponse.error(error.message, null, 500);
    }
  }
  ...
  @ApiParam({ name: 'id', required: true, description: 'Progress Update MongoDB ID' })
  @ApiResponse({ status: 200, type: ProgressUpdateResponseDto, description: 'Progress update retrieved successfully.' })
  ...
  @ApiResponse({ status: 200, type: ProgressUpdateResponseDto, description: 'Progress update updated successfully.' })
  @ApiResponse({ status: 404, description: 'Progress update not found.' })
  async update(@Param('id') id: string, @Body() updateProgressUpdateDto: UpdateProgressUpdateDto) {
    try {
      const update = await this.progressUpdateService.update(id, updateProgressUpdateDto);
      const mapped = ProgressUpdateMapper.toResponse(update);
  ...
  @ApiParam({ name: 'id', required: true, description: 'Progress Update MongoDB ID' })
  @ApiResponse({ status: 200, type: ProgressUpdateResponseDto, description: 'Progress update deleted successfully.' })
  @ApiResponse({ status: 404, description: 'Progress update not found.' })
