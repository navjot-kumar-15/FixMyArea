...
export class CategoryController {
  ...
  async create(@Body() createCategoryDto: CreateCategoryDto) {
    try {
      ...
      return CustomResponse.success(CategoryMapper.toResponse(category), MESSAGES.CATEGORY.CREATED);
    } catch (error) {
  ...
  @ApiQuery({ name: 'all', required: false, type: Boolean, description: 'If true, returns inactive categories as well' })
  ...
  async findAll(@Query('all') all?: string) {
    try {
      ...
      return CustomResponse.success(CategoryMapper.toResponseList(categories), MESSAGES.CATEGORY.FETCHED_ALL);
    } catch (error) {
      return CustomResponse.error(error.message, null, 500);
    }
  }
  ...
  async findOne(@Param('id') id: string) {
    try {
      ...
      return CustomResponse.success(CategoryMapper.toResponse(category), MESSAGES.CATEGORY.FETCHED);
    } catch (error) {
  ...
  async update(@Param('id') id: string, @Body() updateCategoryDto: UpdateCategoryDto) {
    try {
      ...
      return CustomResponse.success(CategoryMapper.toResponse(category), MESSAGES.CATEGORY.UPDATED);
    } catch (error) {
      if (error instanceof ConflictException) {
        return CustomResponse.error(error.message, null, 409);
      }
  ...
  async remove(@Param('id') id: string) {
    try {
      ...
      return CustomResponse.success(CategoryMapper.toResponse(category), MESSAGES.CATEGORY.DELETED);
    } catch (error) {
      if (error instanceof NotFoundException) {
        return CustomResponse.error(error.message, null, 404);
      }
